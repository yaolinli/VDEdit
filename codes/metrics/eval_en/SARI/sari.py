...
def compute(sources, predictions, references):
    ...
    for src, pred, refs in zip(sources, predictions, references):
        ss, keep, delete, add = SARIsent(normalize(src), normalize(pred), [normalize(sent) for sent in refs])
        sari_score += ss
        avgkeepscore += keep
        avgdelscore += delete
        avgaddscore += add
    ...
    return 100 * sari_score, avgkeepscore, avgdelscore, avgaddscore
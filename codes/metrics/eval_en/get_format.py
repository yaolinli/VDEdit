...
for jterm in tqdm(rawdatas):
    ...
    if "newcap_gt" not in jterm:
        jterm["newcap_gt"] = jterm["newcap"]
    if isinstance(jterm["newcap_gt"][0], list):
         jterm["newcap_gt"] = jterm["newcap_gt"][0]
...
for i, dtype in enumerate(dtypes):
    ...
    for i, jterm in enumerate(v):
        ...
        assert isinstance(gtcaps, list) == True
        assert len(gencap) > 1, "generated description can not be none "
        gtcaps = [gtcaps]
        img_data.append({"filename":jterm["vid"], "id":vid})
        gen_data.append({"image_id": vid, "caption": gencap})

        for gtcap_list in gtcaps:
            for gtcap in gtcap_list:
                json_item = {}

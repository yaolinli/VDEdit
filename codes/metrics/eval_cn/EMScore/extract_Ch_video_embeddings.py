...
def encode_video(video_file, preprocess, model):
    ...
    image_input = preprocess(images=images, return_tensors="pt")['pixel_values'].to(device)
    image_features_list = []
    bs = 256
    with torch.no_grad():
        ...
        for i in tqdm(range(n_inter), desc='encoding vid: {}'.format(video_file)):
            image_inputs = {}
            image_inputs['pixel_values'] = image_input[i*bs: (i+1)*bs]
...
if __name__ == "__main__":
    ...
    parse.add_argument('--split', type=str, default='test', help='training,validation,test')
    opt = parse.parse_args()

    vids = [] # vids to extract in videos_path
    file_root = '/home/yaolinli/dataset/EMMAD-EDIT/annotations/'
    ...
    vids = list(set(vids))
    print("#video num to be extracted: ", len(vids))

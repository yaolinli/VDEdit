...
def encode_text(vid_caps, model, tokenizer, idf_dict, device):
    ...
    with torch.no_grad():
        text_features = model.get_text_features(**text_inputs, local=True).float() # [4, 512]
...
def process_cn(a, tokenizer=None):
    if tokenizer is not None:
        a = tokenizer(a)['input_ids']
    return set(a)

def get_idf_dict(arr, tokenizer, nthreads=4, language='en'):
    """
    ...
    if language == 'en':
        ...
        process_partial = partial(process_cn, tokenizer=tokenizer)

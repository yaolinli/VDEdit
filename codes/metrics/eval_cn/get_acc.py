...
parser.add_argument('--dtype', type=str, default='breakdown', help='overall or breakdown')
...
for dtype in dtypes:
    ...
    for i, jterm in tqdm(enumerate(datas)):
        vid = "input" + str(i) + "_" + str(jterm["vid"])
        ...
        dtype = jterm["dtype"]
        assert isinstance(gencap, str) == True
        assert isinstance(gtcaps, str) == True
        assert isinstance(reference, str) == True
        assert isinstance(oldcap, str) == True
        gtcaps = [gtcaps]
        ...
        elif command in ["<dele>", "<keep>"] and (clens0 - clens1) >= N:
            length_change.append(1)
        else:
        ...
        if ("attr" in dtype) :
            ...
            for attr_i in attrs:
                ...
                elif command in ["<dele>", "<keep>"] and not_in(attr_i, gencap):
                    attr_include[atype].append(1) 
                    good_cases.append(jterm)
                else:
